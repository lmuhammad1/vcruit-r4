class VideoPagesController < ApplicationController
  ...
  def index
    @video_pages = VideoPage.where(:profile_id => current_user.profile).paginate(page: params[:page], :per_page => 7).order("updated_at DESC").all
  end
  ...
  def create
    ...
    respond_to do |format|
      if @video_page.save
        ...
        format.html { redirect_to video_pages_url, notice: 'Video page was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @video_page.update(video_page_params)
        format.html { redirect_to video_pages_url, notice: 'Video page was successfully updated.' }
